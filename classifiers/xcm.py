...
from keras.layers import Activation, BatchNormalization, concatenate, Dense, GlobalAveragePooling1D, Input, Reshape
...
def xcm(input_shape, n_class, window_size, filters_num = 128):
    ...
    a = Conv2D(filters=int(filters_num), kernel_size=(int(window_size*n), 1), strides=(1, 1), padding='same', input_shape=(n, k, 1), name='2D')(input_layer)
    ...
    b = Conv1D(filters=int(filters_num), kernel_size=int(window_size*n), strides=1, padding='same', name='1D')(b)
    ...
    z = Conv1D(filters=filters_num, kernel_size=int(window_size*n), strides=1, padding='same', name='1D_Final')(z)
